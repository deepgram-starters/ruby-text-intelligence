# ruby-text-intelligence

Ruby (Sinatra) demo app for Deepgram Text Intelligence.
...
| File | Purpose |
|------|---------|
| `app.rb` | Main backend — API endpoints and request handlers |
...
```
feat(ruby-text-intelligence): add diarization support
fix(ruby-text-intelligence): resolve WebSocket close handling
refactor(ruby-text-intelligence): simplify session endpoint
